...
>
> 除了监视，看板还能动手：**「＋ 新任务」**直接下发任务（选目录/权限模式/写目标，与手机端同协议）；任务卡片**点标题展开详情**（模式/模型/任务原文、完整流式回复、待发送队列内容、事件时间线，可直接「停止」）；底栏 **🔔 提醒**开关可在任务完成时弹系统通知。运行中的任务显示「开始时刻 · 已运行时长」（桥接重启也会从日志回放真实起点）。开发时可跑 `node test/mini-mock.mjs` 起一个假数据模拟器验证页面。
